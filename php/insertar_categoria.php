...
<body>
    ...
    <div class="category-insert container px-3 py-3 pt-md-5 pb-md-4 mt-md-5 mt-md-4 mx-auto">
        <div class="mb-md-4 mb-4">
            <h1>Add category</h1>
            <h2>Use the following form to add a category to the DB</h2>
            <?php
            if ($conn->connect_error) {
                echo "An error occurred trying to connect to the DB";
            } else {
                if (isset($_POST["nomCategoria"])) {
                    $sql = "SELECT * FROM categories WHERE CategoryName='" . $_POST["nomCategoria"] . "'";
                    $results = $conn->query($sql);
                    if ($results->num_rows > 0) {
                        echo "A category with given name already exists";
                    } else {
                        $sql = "INSERT INTO categories (CategoryName,Description) VALUES ('" . $_POST["nomCategoria"] . "','" . $_POST["descripcio"] . "')";
                        if ($conn->query($sql)) {
                            echo "Category created successfully";
                        } else {
                            echo "An error occurred while creating the category";
                        }
                    }
                }
            }
